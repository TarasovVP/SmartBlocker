...
class DetailsNumberDataUIModelUseCaseTest {
    ...
    fun filteredCallsByNumberTest() = runBlocking {
        val filteredCallList = listOf(FilteredCallWithFilter().apply { callUIModel = FilteredCall().apply { this.number =
            TEST_NUMBER
        } })
        ...
        assertEquals(TEST_NUMBER, (result[0] as FilteredCallWithFilter).callUIModel?.number)
    }
