...
class LogCallRepositoryUnitTest {
    ...
    fun getSystemLogCallListTest() =
        runBlocking {
            ...
            } returns logCall.phoneNumberValue.orEmpty()
    ...
    @Test
    fun allCallWithFiltersByFilterTest() =
        runBlocking {
            val logCallList =
                listOf(
                    CallWithFilter().apply {
                        call = LogCall(callId = 1).apply { number = "1" }
            ...
            val expectedResult =
                logCallList.filter { it.filterWithFilteredNumber?.filter?.filter == TEST_FILTER }
