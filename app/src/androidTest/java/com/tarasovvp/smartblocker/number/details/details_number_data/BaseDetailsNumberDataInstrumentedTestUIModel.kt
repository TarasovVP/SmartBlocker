...
open class BaseDetailsNumberDataInstrumentedTestUIModel: BaseInstrumentedTest() {
    ...
    override fun setUp() {
        ...
        launchFragmentInHiltContainer<DetailsNumberDataFragment> (fragmentArgs = bundleOf("numberData" to numberData)) {
            ...
            contactWithFilter = if (numberDataUIModelFromBundle is CallWithFilterUIModel) ContactWithFilter(filterWithCountryCode = numberDataUIModelFromBundle.filterWithCountryCode,
                contact = Contact(name = getString(R.string.details_number_from_call_log),
                    photoUrl = numberDataUIModelFromBundle.callUIModel?.photoUrl,
                    number = numberDataUIModelFromBundle.callUIModel?.number.orEmpty(),
                    filter = numberDataUIModelFromBundle.filterWithCountryCode?.filter?.filter.orEmpty())) else numberDataUIModelFromBundle as ContactWithFilter
