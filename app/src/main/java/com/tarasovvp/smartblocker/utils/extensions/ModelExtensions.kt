...
fun Context.numberDataFilteringText(filterIndexes: ArrayList<Int>): String {
    return if (filterIndexes.isEmpty()) {
...
fun NumberDataUIModel.highlightedSpanned(
                ...
                ->
                    number.highlightedSpanned(
                        filterWithFilteredNumberUIModel?.countryCode?.takeIf {
                            ...
                            ?.let { filterWithFilteredNumberUIModel.filter.substring(it.length) },
