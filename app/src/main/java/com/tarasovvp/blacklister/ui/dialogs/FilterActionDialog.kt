...
class FilterActionDialog : BaseDialog<DialogInfoBinding>() {
    ...
    override fun initUI() {
        val filterActionText = when(args.filterAction) {
            AddFilterState.ADD_FILTER_ADD.name -> "Добавить ${args.filter?.filter.orEmpty()} в ${getString(if (args.filter?.isBlackFilter().isTrue()) R.string.black_list else R.string.white_list)}"
            AddFilterState.ADD_FILTER_DELETE.name -> "Удалить ${args.filter?.filter.orEmpty()} из ${getString(if (args.filter?.isBlackFilter().isTrue()) R.string.black_list else R.string.white_list)}"
            AddFilterState.ADD_FILTER_CHANGE.name -> "Переместить ${args.filter?.filter.orEmpty()} в ${getString(if (args.filter?.isBlackFilter().isTrue()) R.string.white_list else R.string.black_list)}"
