...
interface ContactDao {
    ...
    @Query("SELECT * FROM contact WHERE (trimmedPhone = :number) OR (trimmedPhone LIKE '%' || :number || '%' AND :contain) OR (trimmedPhone LIKE :number || '%' AND :start) OR (trimmedPhone LIKE '%' || :number AND :end)")
